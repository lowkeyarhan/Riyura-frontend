...
export interface LiquidEtherProps {
  ...
  isBounce?: boolean;
  colors?: string[];
  /** Override palette slot 0–2 when set (merged with `colors`). */
  color0?: string;
  color1?: string;
  color2?: string;
...
const defaultColors = ["#3d9a60", "#8fd4a8", "#dff5e8"];
...
function resolvePalette(
  ...
  const base =
    Array.isArray(colors) && colors.length > 0
      ? [...colors]
      : [...defaultColors];
  const norm = base.length === 1 ? [base[0]!, base[0]!] : base.slice(0, 3);
  while (norm.length < 3) {
    norm.push(norm[norm.length - 1] ?? defaultColors[norm.length]);
  }
  // `color0`–`color2` override matching slots (React Bits API).
  return [color0 ?? norm[0]!, color1 ?? norm[1]!, color2 ?? norm[2]!];
}
...
export default function LiquidEther({
  ...
  useEffect(() => {
    ...
    const paletteStops = resolvePalette(colors, color0, color1, color2);
...
`;
    ...
    class ExternalForce extends ShaderPass {
      ...
      init(simProps?: Record<string, unknown>) {
        ...
        const cursorSz = simProps?.cursor_size as number;
        const mouseM = new THREE.RawShaderMaterial({
          ...
          uniforms: {
            ...
            scale: { value: new THREE.Vector2(cursorSz, cursorSz) },
          },
        });
      ...
      update(...args: unknown[]) {
        ...
        uniforms.scale.value.set(cursorSz, cursorSz);
        super.update();
      }
    }
